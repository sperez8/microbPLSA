...
outnodesfile = _root_dir + '/nodesmod' + str(NUM_AXIS) + 'pos' + '.js'
outlinksfile = _root_dir + '/WebContent/linksmod' + str(NUM_AXIS) + 'pos' + '.js'
innodesfile = _root_dir + '/Data/WL_Nodes_EnsembleNetwork.csv'
inlinksfile = _root_dir + '/Data/WL_ensemble_POSEDGES.csv'

def get_nodes(file):
	'''returns node data from csv file'''
	data = np.loadtxt(file, delimiter=delimiter, skiprows = 1, usecols = (0,1,2,4,5))
	nodes = list(data[:,0])
...
def write_nodes(file, nodes, positions, axis, modulegroup):
	...
	for n in nodes:
		f.write('  {x: '+str(axis[n])+', y: '+str(positions[n])+', z: '+str(modulegroup[n])+'},\n')
	f.write('];')
